...
<script type="text/javascript">
	...
	function searchData(){
		...
		// レスポンスを受けて（検索結果のリスト）によって、動的にhtmlタグを作って表を作る
			...
			$.each(searchResults , function(i){
				...
				if(identificationNum=="A5"){
					 $("#bodyTableInfo").append($('<tr onclick="goInsert("A5","10")>')   								   
		...
		a.title = '登録';
		a.href = "/HappyBeautyManager/manage/registrate";
		a.id = 'searchButton';
		a.setAttribute('class', 'btn');
		//a.setAttribute('class', 'btn btn-primary');
		document.getElementById("registrate").appendChild(a);
	}
	
	['hashchange', 'load'].forEach(event => window.addEventListener(event, searchForm));
	
function goInsert(identificationNum,stampEventId){
	console.log(identificationNum);
	console.log(stampEventId);
		$("#bodyTableInfo").append($('<input type="hidden" name="identificationNum" value="identificationNum"/>'))	
		 				 .append($('<input type="hidden" name="stampEventId" value="+stampEventId"/>'))
...
<body>
	...
	<div class="container-">
        ...
        <br> <br>
        <div class="container">
				<form:form action="${pageContext.request.contextPath}/manage/updateProcess" method="POST" id="updateForm" modelAttribute="updateForm" >
				<table border="1" class="table table-striped table-bordered">
            ...
            <div id="registrate" style="width:100px;"></div>
        </div>
